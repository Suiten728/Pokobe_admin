...
# JSONロード関数
def load_posts() -> dict:
    with open("data_public/texts.json", "r", encoding="utf-8") as f:
        return json.load(f)
...
class LanguageSelect(discord.ui.Select):
    def __init__(self, item_name: str, item_data: dict):
        ...
        super().__init__(
            ...
            # 各項目専用の unique custom_id
            custom_id=f"dynamic_lang_select_{item_name}"
        )
...
class PostView(discord.ui.View):
    def __init__(self, item_name: str, item_data: dict):
        ...
        self.add_item(LanguageSelect(item_name, item_data))
...
class DynamicPostCog(commands.Cog):
    ...
    async def post(self, ctx: commands.Context, item_name: str):
        ...
        if item is None:
            await ctx.send(f"❌ `{item_name}` は存在しません。")
            return

        # デフォルトは日本語。なければ最初の言語
        text = item.get("ja") or next(iter(item.values()))

        await ctx.send(text, view=PostView(item_name, item))

    @commands.Cog.listener()
    async def on_ready(self):
        # 再起動後用： 全ての View を登録
        for name, item_data in self.data.items():
            try:
                self.bot.add_view(PostView(name, item_data))
            except:
                pass
